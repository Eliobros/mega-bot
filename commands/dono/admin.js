...
async promover(msg, args, groupJid, senderJid) {
    const config = this.getConfig();
    ...
    try {
        ...
        // Verifica se quem executa o comando é admin ou dono
        ...
        if (!botIsAdmin) {
            await this.sendMessage(groupJid, '❌ Eu preciso ser administrador para promover membros!');
            return;
        }

        // Descobre quem é o alvo (menção ou reply)
        ...
        if (mentionedJids.length > 0) {
            ...
            await this.sendMessage(groupJid, '❌ Mencione alguém ou responda a uma mensagem para promover!');
            return;
        }
        ...
        if (targetParticipant?.admin) {
            ...
            await this.sendMessage(groupJid, `⚠️ @${targetNum} já é administrador!`, { mentions: [targetJid] });
            return;
        }
        ...
        // Mensagem de sucesso
        const targetNum = targetJid.replace('@s.whatsapp.net', '');
        const promoterNum = senderJid.replace('@s.whatsapp.net', '');

        let successMsg = `👑 *USUÁRIO PROMOVIDO!*\n\n`;
        successMsg += `✅ @${targetNum} agora é administrador\n`;
        successMsg += `👤 Promovido por: @${promoterNum}\n`;
        successMsg += `📅 Data: ${new Date().toLocaleString('pt-BR')}\n`;
        successMsg += `🎉 Parabéns pela promoção!`;
        ...
        await this.sendMessage(groupJid, '❌ Erro ao promover usuário!');
    }
}
